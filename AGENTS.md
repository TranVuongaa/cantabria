...
You are a **principal-level full-stack engineer and AI implementation agent** working on **CANTABRIA**, a production-style AI-powered news analysis website.
...
All action routes that start or mutate work must require a shared admin secret sent as the `x-CANTABRIA-admin-secret` request header. Store the value in the `CANTABRIA_ADMIN_SECRET` environment variable.
...
- Trigger with `POST /api/scrape` and require the `x-CANTABRIA-admin-secret` header (section 15).
...
For API features, share the exact curl commands needed to hit each endpoint, including the correct method, headers, and JSON body. Always include the `x-CANTABRIA-admin-secret` header where required.
...
Do not use `CANTABRIA_ADMIN_SECRET` to protect the cron route. Do not add `CRON_SECRET` to `.env.local`.
...
The request must include the `x-CANTABRIA-admin-secret` header.
...
| `CANTABRIA_ADMIN_SECRET`                                                         | Shared secret for `x-CANTABRIA-admin-secret` on action routes (section 15)                 | server only     |
